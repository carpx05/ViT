...
class PatchEmbedding(nn.Module):
    """Create 1D Sequence learnable embedding vector from a 2D input image
    Args:
    ...
    def forward(self, x):
        ...
        assert image_size % self.patch_size == 0, f"Input image size must be divisible by patch size, image shape: {image_size}, patch size: {self.patch_size}"
...
if __name__ == "__main__":
    ...
    test_patcher = PatchEmbedding(in_channels=in_channels, patch_size=patch_size, embedding_dim=embedding_dimension)
    ...
    try:
        ...
        print("\033[1;32m VViT: Tests passed!")
    except:
